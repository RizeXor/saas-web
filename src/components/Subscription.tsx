      ...
      <div className="card rounded">
        ...
        <div className="card-body">
          ...
          <p className="card-text">
            Billed: {plan.currency === "usd" ? "$" : plan.currency}
            {plan.amount.toString().slice(0, -2) +
              "." +
              plan.amount.toString().slice(-2)}{" "}
            every {plan.interval}
          </p>
        </div>
        <div className="card-footer text-white-50 d-flex justify-content-between">
          <small>
            Expires on {dayjs.unix(current_period_end).format("DD MMM YYYY")}
          </small>
          <small>
            {days_until_due} {days_until_due === 1 ? "day" : "days"} left
          </small>
        </div>
      </div>
    </div>
